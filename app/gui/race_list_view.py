...
class RaceListView(QWidget):
    ...
    def __init__(self):
        ...
        self.table.setHorizontalHeaderLabels(["主催", "競馬場", "R", "距離", "馬場", "発走", "頭数"])
    ...
    def load_races(self):
        ...
        if not races:
            fetch_for_date(d, org)
            snapshot_odds(d, mode="prevday_last", org=org)
            races = db.fetch_races(date=d, org=org)

        self._races = races
        self.table.setRowCount(len(races))
        for i, r in enumerate(races):
            vals = [r["org"], r["venue"], f"{r['race_no']}R", str(r.get("distance_m") or ""), r.get("surface") or "", r.get("start_time") or "", f"{r.get('field_size') or ''}"]
